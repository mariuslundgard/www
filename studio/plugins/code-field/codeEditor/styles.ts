...
export function codeEditorStyle(props: {$padding: number; theme: Theme}) {
  const {$padding, theme} = props
  ...
  return css`
    ...
    & .CodeMirror pre.CodeMirror-line-like {
      padding: 0 ${rem(space[$padding])};
    }

    /* Sizer */
    & .CodeMirror-sizer {
      border-right-width: ${rem(space[$padding])} !important;
    }
  `
}
